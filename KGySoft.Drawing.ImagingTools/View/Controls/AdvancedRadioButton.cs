...
{
    /// <summary>
    /// A RadioButton that fixes the rendering in dark mode (see also ControlExtensions). Issue: with System FlatStyle enabled text is too dark, whereas with Standard FlatStyle the disabled text is too dark.
    /// NOTE: Using the same Enabled/Disabled change as in AdvancedCheckBox for symmetry, though in this project we don't have disabled RadioButtons.
    /// </summary>
    internal class AdvancedRadioButton : RadioButton
    {
        #region Fields
        ...
        {
            // Only when using System FlatStyle (this could be removed when using AdvancedRadioButton from KGySoft.WinForms)
